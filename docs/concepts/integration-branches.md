...
becomes the automatic target for every stage: workers spawn their worktrees
from the integration branch (so they start with sibling work already present),
the Merger merges completed MRs into the integration branch instead of main,
and when all epic children are closed, the Merger can land the integration
...
Landing can happen on command or automatically via sweep. The result is that
an entire epic flows through the system as a coherent unit, from first dispatch
...
3. **Create a batch to track the work.** The batch gives you a single dashboard
   for the entire epic's progress.
   ```bash
   gt batch create "Auth overhaul" gt-auth-tokens gt-auth-sessions gt-auth-middleware
   ```

4. **Dispatch the first wave.** Identify children with no blockers and dispatch them
   to the project. Use `--no-batch` since the tracking batch already exists.
   ```bash
   gt dispatch gt-auth-tokens gastown --no-batch
   gt dispatch gt-auth-sessions gastown --no-batch
   ```

5. **Workers process the work.** Each worker spawns its worktree from the
   integration branch, so it starts with any sibling work that has already
   landed there. When a worker finishes, it submits a merge request.

6. **Merger merges to the integration branch.** Instead of merging to main,
   the Merger merges each MR into the integration branch and marks the child
   task as complete.

7. **Track progress via the batch.** The batch status updates each time the
   Merger completes a task.
   ```bash
   gt batch status hq-cv-abc
   ```

8. **Dispatch the next wave.** When a wave completes and its dependent children
   unblock, dispatch the next batch. Those workers will start from the integration
   branch — which now contains all the work from the preceding wave.
   ```bash
   gt dispatch gt-auth-middleware gastown --no-batch
   ```
...
9. **Land when complete.** When all children under the epic are closed, the
   ...
   enabled, the Merger does this automatically during sweep. Otherwise,
   land manually:
   ```bash
...
Assign children to workers as normal:

```bash
gt dispatch gt-auth-tokens gastown
gt dispatch gt-auth-sessions gastown
```

Workers auto-detect the integration branch when their issue is a child of an
epic that has one. No manual targeting needed.

### 4. MRs Merge to Integration Branch

When workers run `gt done` or `gt mq submit`, auto-detection kicks in:
...
The Merger processes these MRs and merges them to the integration branch.
...
|--------|-------------|-------------|
| `gt done` / `gt mq submit` | Targets MR at integration branch instead of main | `integration_branch_merger_enabled` |
| Worker spawn | Sources worktree from integration branch | `integration_branch_worker_enabled` |
| Merger sweep | Checks if integration branches are ready to land | `integration_branch_auto_land` |
...
| Step | Action | Result |
|------|--------|--------|
| 1 | Load config, check `integration_branch_merger_enabled` | If false, skip detection |
...
| `--base-branch` | Create from this branch instead of the project's default branch (also sets where `land` merges back to) | `origin/<default_branch>` |
...
2. Reads base branch from epic metadata (defaults to the project's `default_branch` if not stored)
...
The project's `default_branch` (set in `config.json`, auto-detected during `gt project add`)
...
`develop` or `master` instead of `main`, set it once in project config and the whole
pipeline follows:
...
All integration branch fields live under `merge_queue` in project settings (`settings/config.json`):
...
| `integration_branch_merger_enabled` | `*bool` | `true` | `gt mq submit` and `gt done` auto-detect integration branches as MR targets |
...
(true for worker/merger enabled, false for auto-land). Set explicitly to `false`
to disable.

## Auto-Landing

When `integration_branch_auto_land` is `true`, the Merger sweep automatically
lands integration branches that are ready.

### How It Works

During each sweep cycle, the Merger:
...
- `integration_branch_merger_enabled: true` (integration feature is on)
- `integration_branch_auto_land: true` (auto-landing is on)

If either is false, the sweep step exits early.
...
### Layer 1: Template and Role Instructions

The merger template and role template explicitly forbid landing integration
...
(fallback: `main`), so this works regardless of the project's branch naming.
...
are not covered by the hook — Layer 1 (template language) is the guardrail for
those cases.

**Requires**: `core.hooksPath` must be configured for the hook to be active.
New projects get this automatically. Existing projects: run `gt doctor --fix`.
...
|-------|------|----------|------------|
| Template/Role | Soft | Covers all branch patterns | AI agents can ignore instructions |
...
The layers complement each other. The template covers custom templates; the hook
...
Integration branches work with different project toolchains. The project's build pipeline
commands are auto-injected into worker-work, merger-sweep, and sync-workspace
templates so agents know how to validate work for each project.
...
**Go project** (all commands empty by default — configure per-project):
```json
{
  "merge_queue": {
...
### How Commands Flow Into Templates

Commands are auto-injected from `<project>/settings/config.json` into template vars:

- **Merger sweep**: `buildMergerPatrolVars()` reads project config during `gt prime`
- **Worker work / sync**: `loadRigCommandVars()` reads project config during `gt dispatch`

User-provided `--var` flags on `gt dispatch` override project config values.

### Empty = Skip

Any command left empty (or not configured) is skipped silently by the template.
This means a Go project doesn't need `setup_command` or `typecheck_command`, and a
TypeScript project can add all five without affecting Go projects.

Workers working on integration branches inherit the project's build pipeline
...
**Wrong:** Dispatch children, then create the integration branch later.

Children dispatched before the integration branch exists will target main. Their MRs
won't flow to the integration branch. Create the integration branch *first*,
before dispatching any child work.
...
- `integration_branch_merger_enabled` is not `false`
...
- [Worker Lifecycle](worker-lifecycle.md) — How workers submit to the merge queue
