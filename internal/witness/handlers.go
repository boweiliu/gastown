...
func (c ZombieClassification) ImpliesActiveWork() bool {
	...
	case ZombieStuckInDone, ZombieAgentDeadInSession, ZombieBeadClosedStillRunning,
		...
		ZombieNeverHeartbeated, ZombieSubmittedStillRunning:
		return true
	default:
		return false
	}
...
func detectZombieLiveSession(bd *BdCli, workDir, townRoot, rigName, polecatName, sessionName string, t *tmux.Tmux, doneIntent *DoneIntent, witCfg *config.WitnessThresholds, snap *agentBeadSnapshot) (ZombieResult, bool) {
	...
	// but fail before exiting the polecat session. If the hook is still open and
	// there is completion/MR evidence, nudge the live session to finish instead of
	// letting it sit idle forever.
...
func detectSubmittedStillRunning(bd *BdCli, workDir, polecatName, sessionName string, t *tmux.Tmux, hb *polecat.SessionHeartbeat, snap *agentBeadSnapshot, staleThreshold time.Duration) (ZombieResult, bool) {
	...
	age, shouldNudge := isSubmittedStillRunningCandidate(snap, hb, hookSt, staleThreshold)
	if !shouldNudge {
		return ZombieResult{}, false
	}
	...
	zombie := ZombieResult{
		...
		CleanupStatus:  snap.cleanupStatus(),
		WasActive:      true,
		Action:         fmt.Sprintf("nudged-exit-submitted-session (idle=%v, hook_status=%s)", age.Round(time.Second), hookSt),
	}
	msg := fmt.Sprintf("RECOVERY_NEEDED: gt done appears submitted (hook=%s, cleanup_status=clean), but this session is still running with no fresh heartbeat for %v. If work is already submitted, exit now; otherwise run gt done again.", snapHook, age.Round(time.Second))
...
func isSubmittedStillRunningCandidate(snap *agentBeadSnapshot, hb *polecat.SessionHeartbeat, hookStatus string, staleThreshold time.Duration) (time.Duration, bool) {
	if snap == nil || snap.HookBead == "" || snap.cleanupStatus() != "clean" {
		return 0, false
	}
	if !isOpenHookStatus(hookStatus) || !hasSubmittedWorkEvidence(snap) {
		return 0, false
	}
...
func hasSubmittedWorkEvidence(snap *agentBeadSnapshot) bool {
	if snap == nil {
		return false
	}
	if beads.AgentState(snap.AgentState) == beads.AgentStateDone || snap.ActiveMR != "" {
		return true
	}
	if snap.Fields == nil {
		return false
	...
	return snap.Fields.ActiveMR != "" || snap.Fields.MRID != "" ||
		(snap.Fields.ExitType == string(ExitTypeCompleted) && snap.Fields.CompletionTime != "")
}
