...
func runNamepool(cmd *cobra.Command, args []string) error {
	...
	if err == nil && settings.Namepool != nil {
		...
		// Use defaults
		pool = worker.NewNamePool(rigPath, rigName)
	}
	...
	fmt.Printf("Rig: %s\n", rigName)
	theme := pool.GetTheme()
	if worker.IsBuiltinTheme(theme) {
...
func runNamepoolThemes(cmd *cobra.Command, args []string) error {
	...
	if len(args) == 0 {
		...
		themes := worker.ListAllThemes(townRoot)
		...
		for _, t := range themes {
			...
			if t.IsCustom && townRoot != "" {
				names, _ = worker.ResolveThemeNames(townRoot, t.Name)
			} else {
				names, _ = worker.GetThemeNames(t.Name)
			}
			if len(names) > 0 {
				preview := names
				if len(preview) > 10 {
	...
	var err error
	if townRoot != "" {
		names, err = worker.ResolveThemeNames(townRoot, theme)
	} else {
		names, err = worker.GetThemeNames(theme)
	}
	if err != nil {
...
func runNamepoolSet(cmd *cobra.Command, args []string) error {
	...
	if _, err := worker.ResolveThemeNames(townRoot, theme); err != nil {
		return fmt.Errorf("unknown theme: %s (use 'gt namepool themes' to list available themes)", theme)
	}
...
func runNamepoolAdd(cmd *cobra.Command, args []string) error {
	...
	// Validate name
	if err := worker.ValidatePoolName(name); err != nil {
		return err
	}
	...
	if style != "" && !worker.IsBuiltinTheme(style) && len(settings.Namepool.Names) == 0 {
		townRoot, _ := workspace.FindFromCwd()
		if townRoot != "" {
			alreadyExists, err := worker.AppendToCustomTheme(townRoot, style, name)
			if err != nil {
...
func runNamepoolCreate(cmd *cobra.Command, args []string) error {
	...
	if namepoolFromFileFlag != "" {
		// Read from file
		names, err = worker.ParseThemeFile(namepoolFromFileFlag)
		if err != nil {
	...
	if err := worker.SaveCustomTheme(townRoot, themeName, names); err != nil {
		return err
	}
...
func runNamepoolDelete(cmd *cobra.Command, args []string) error {
	...
	if using := worker.FindRigsUsingTheme(townRoot, themeName); len(using) > 0 {
	...
	if err := worker.DeleteCustomTheme(townRoot, themeName); err != nil {
		return err
	}
