...
func TestOutputRoleDirectives(t *testing.T) {
	...
	t.Run("town-level directive emits town header", func(t *testing.T) {
		...
		if err := os.WriteFile(filepath.Join(dir, "worker.md"), []byte("Always be polite."), 0644); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("both levels emits combined header", func(t *testing.T) {
		...
		if err := os.WriteFile(filepath.Join(townDir, "worker.md"), []byte("Town rule."), 0644); err != nil {
			t.Fatal(err)
		}
		...
		if err := os.WriteFile(filepath.Join(rigDir, "worker.md"), []byte("Rig rule."), 0644); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("explain mode shows file paths", func(t *testing.T) {
		...
		if !strings.Contains(out, filepath.Join("directives", "worker.md")) {
